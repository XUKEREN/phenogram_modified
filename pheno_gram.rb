...
class ExhaustiveSearchColorMaker < ColorMaker
	...
	def initialize
		@index=0
		@exhaustiveColors = ['rgb(255,0,0)','rgb(0,160,138)','rgb(242,173,0)','rgb(249,132,0)','rgb(91,188,214)','rgb(236,203,174)',
...
def draw_plot(genome, phenoholder, options)
  ...
  if eth_shapes.length > 1 and !options.chr_only
		eth_label_row_size = circle_size * label_offset_y*1.2 # Keren added more space between shapekey and chromesome
		max_ethlength=0
  ...
  options.zoomchr ? total_x = x_per_chrom * 4 + padded_width*2 : total_x = x_per_chrom * chroms_in_row + padded_width * 2
	...
	for i in 2..chroms_in_row+1
		if options.include_notes
			...
			options.big_font ? chars_per_circle=1.0 : chars_per_circle=1.1 # Keren changed 1.2 to 1.1 to make chrs more spread
